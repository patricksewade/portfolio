...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-display font-bold text-slate-900"><?= e($page_title) ?></h1>
        <a href="<?= e(BASE_URL) ?>/admin/dashboard" class="text-slate-500 hover:text-slate-800 transition-colors">
    ...
    <form action="<?= e($action_url) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4" required
                class="w-full px-4 py-3 rounded-lg border border-slate-200 focus:ring-2 focus:ring-brand focus:border-brand transition-colors bg-slate-50 focus:bg-white"><?= e($project['description'] ?? '') ?></textarea>
        </div>
        ...
        <div>
            ...
            <?php if (!empty($project['image_url'])): ?>
                <div class="mb-3">
                    ...
                    <img src="<?= e($project['image_url']) ?>" alt="Aperçu" class="h-24 w-auto rounded border border-slate-200 shadow-sm mb-2">
            ...
            <p class="mt-1 text-xs text-slate-500">Formats acceptés : JPG, PNG, GIF, WEBP. Laissez vide pour conserver l'image actuelle.</p>
        </div>

        <!-- Is Featured -->
        <div class="flex items-center">
            <input id="is_featured" name="is_featured" type="checkbox" value="1" <?= !empty($project['is_featured']) ? 'checked' : '' ?>
        ...
        <div class="pt-6 border-t border-slate-100 flex justify-end space-x-4">
            <a href="<?= e(BASE_URL) ?>/admin/dashboard" class="px-6 py-3 border border-slate-300 shadow-sm text-sm font-medium rounded-lg text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand transition-colors">
                Annuler
            </a>
            <button type="submit" class="px-6 py-3 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-brand hover:bg-brand/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand transition-colors">
                <?= empty($project['id']) ? 'Ajouter le projet' : 'Enregistrer les modifications' ?>
            </button>
        </div>
    </form>
</div>
