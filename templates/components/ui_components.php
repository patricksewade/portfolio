...
function render_project_card(array $project): void {
    $featuredClass = !empty($project['is_featured']) ? 'ring-2 ring-blue-500 bg-blue-50/30' : 'border border-slate-200 hover:border-slate-300';
    $badge = !empty($project['is_featured']) ? '<span class="absolute top-4 right-4 px-3 py-1 text-xs font-extrabold bg-blue-600 text-white rounded-full shadow-md z-10">⭐ PROJET PHARE</span>' : '';
    ...
    <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all p-5 flex flex-col h-full relative group <?= $featuredClass ?>">
        ...
        <!-- Espace Image / Logo -->
        <div class="w-full h-48 mb-5 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 overflow-hidden group-hover:border-blue-200 transition-colors relative">
            <?php if ($imageUrl): ?>
        ...
        <h3 class="text-xl font-bold mb-2 text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-2"><?= e($project['title']) ?></h3>
        <p class="text-slate-600 mb-5 flex-grow text-sm leading-relaxed line-clamp-3"><?= nl2br(e($project['description'] ?? '')) ?></p>
        
        <div class="text-xs font-bold text-blue-700 mb-5 bg-blue-50/80 w-max px-3 py-1.5 rounded-lg border border-blue-100/50">
        ...
        <div class="flex flex-wrap gap-2 mt-auto pt-4 border-t border-slate-50">
            ...
            <?php if (!empty($project['live_demo_url']) && $project['live_demo_url'] !== '#'): ?>
                <a href="<?= e($project['live_demo_url']) ?>" target="_blank" class="flex-1 px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm text-center">Visiter</a>
            <?php endif; ?>
        </div>
    </div>
    <?php
...
function render_skill_badge(string $skill_name): void {
    ?>
    <span class="inline-flex items-center px-4 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-semibold rounded-full shadow-sm hover:shadow-md hover:border-blue-300 hover:text-blue-600 transition-all cursor-default">
        <?= e($skill_name) ?>
    </span>
    <?php
}
...
function render_experience_timeline(array $exp): void {
    ...
    <div class="relative pl-8 sm:pl-48 py-6 group">
        ...
        <div class="flex flex-col items-center absolute left-2 sm:left-[11rem] h-full top-0">
            <div class="h-6 w-px bg-slate-200 group-hover:bg-blue-300 transition-colors"></div>
            <div class="w-4 h-4 rounded-full bg-white border-4 border-blue-500 z-10 group-hover:scale-125 transition-transform shadow-sm"></div>
            <div class="h-full w-px bg-slate-200 group-hover:bg-blue-300 transition-colors"></div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-start group-last:before:hidden">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group-hover:shadow-md transition-all w-full">
                ...
                <?php if ($exp['company']): ?>
                    <div class="text-blue-600 font-semibold mt-1 mb-3">
                        <?= e($exp['company']) ?> 
                        <?php if ($exp['location']): ?>
...
function render_education_timeline(array $edu): void {
    ...
    <div class="relative pl-8 sm:pl-48 py-6 group">
        ...
        <div class="flex flex-col items-center absolute left-2 sm:left-[11rem] h-full top-0">
            <div class="h-6 w-px bg-slate-200 group-hover:bg-teal-300 transition-colors"></div>
            <div class="w-8 h-8 -ml-1 sm:ml-0 rounded-full bg-teal-50 border-2 border-teal-500 flex items-center justify-center z-10 group-hover:scale-110 group-hover:bg-teal-500 transition-all shadow-sm text-teal-600 group-hover:text-white">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path>
                </svg>
            </div>
            <div class="h-full w-px bg-slate-200 group-hover:bg-teal-300 transition-colors"></div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-start group-last:before:hidden">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group-hover:shadow-md transition-all w-full relative overflow-hidden">
                ...
                <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-teal-50 to-emerald-50 rounded-bl-[100px] -mr-8 -mt-8 opacity-60 transition-transform group-hover:scale-110"></div>
                
                <div class="relative z-10">
                    <h3 class="text-xl font-bold text-slate-900"><?= e($edu['degree']) ?></h3>
                    <div class="text-teal-600 font-bold mt-1 mb-4 text-base">
                        <?= e($edu['specialty']) ?>
                    </div>
