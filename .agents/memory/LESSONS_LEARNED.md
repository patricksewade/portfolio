...
- **Sécurité Git / Secrets** : Un fichier contenant des secrets (`.env`) ne doit jamais être commité. En cas de commit accidentel, un simple `git rm --cached` et un ajout au `.gitignore` ne suffisent pas, car le fichier reste dans l'historique. Il faut purger l'historique complet (ex: `git filter-branch` ou BFG Repo-Cleaner) pour éradiquer la fuite.

## Pièges de l'Environnement (WAMP / Windows)
- **Génération Markdown via PowerShell** : Lors de l'utilisation de `Set-Content` avec des chaînes `here-strings` entre guillemets doubles (`@"`), les backticks markdown (`) sont interprétés comme des caractères d'échappement par PowerShell, ce qui détruit le formatage. Il faut utiliser des `here-strings` à guillemets simples (`@'`) ou l'outil `write_to_file` pour éviter cela.
